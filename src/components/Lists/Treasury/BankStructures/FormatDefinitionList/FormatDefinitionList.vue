<template>
  ...
  >
      ...
      <q-item-section>
        <div class="column">
          ...
          <span class="text-subtitle1 text-grey-14">
            Haz click sobre Definición de formato para desplegar la tabla
          </span>
        </div>
      </q-item-section>
      <q-item-section side>
        <Button
          ...
          :tooltip="
            canCreateFormatDefinition
              ? 'Añadir definición de formato'
              : 'Debes filtrar por un banco primero'
          "
          :stop-propagation="true"
          @click="handleOptions('create')"
          :disabled="!canCreateFormatDefinition"
        />
      </q-item-section>
    </template>
    <TableList
      ref="tableFormatDefinitionList"
    ...
    >
      <template #status="{ row }">
        <ShowStatus v-if="row?.status.id" :type="Number(row?.status.id ?? 1)" />
      ...
      <template #actions="{ row }">
        ...
        <Button
          ...
          colorIcon="#f45100"
          :tooltip="'Ver'"
          @click="handlerGoTo('FormatDefinitionView', row.id)"
        />
        <!-- Editar -->
        <Button
  ...
  >
    <template #default-body>
      <FormatDefinitionForm
        ref="formFormatDefinition"
...
import useFormatDefinitionList from './FormatDefinitionList'
import AlertModalComponent from '@/components/common/AlertModal/AlertModalComponent.vue'
import FormatDefinitionForm from '@/components/Forms/Treasury/BankStuctures/FormatDefinition/FormatDefinitionForm.vue'
