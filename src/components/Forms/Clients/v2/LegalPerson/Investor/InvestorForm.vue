    ...
    <section>
      ...
      <div class="row q-col-gutter-x-lg q-col-gutter-y-sm">
        <div class="col-xs-12 col-sm-12 col-md-6">
          ...
          :default_value="models.quantitative_risk_rating"
          :required="false"
          :disabled="['edit'].includes(action)"
          :rules="[
          ...
          @update:model-value="models.quantitative_risk_rating = $event"
          />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Calificación de riesgo cuantitativo</p>
            <p class="text-weight-medium no-margin">
              {{ models.quantitative_risk_rating ?? 'No registrado' }}
            </p>
          </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-6">
          ...
          :default_value="models.qualitative_risk_rating"
            :required="false"
            :disabled="['edit'].includes(action)"
            :rules="[
              (v: string) => useRules().max_length(v, 60)
            ]"
            @update:model-value="models.qualitative_risk_rating = $event"
            />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Calificación de riesgo cualitativo</p>
            <p class="text-weight-medium no-margin">
              {{ models.qualitative_risk_rating ?? 'No registrado' }}
            </p>
          </div>
        </div>
      </div>
...
//Logic Form
import useInvestorForm from '@/components/Forms/Clients/v2/LegalPerson/Investor/InvestorForm'
