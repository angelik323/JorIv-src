<template>
  <q-form ref="formShareholder" class="q-pa-lg">
    <section>
      <div class="q-mb-lg">
        ...
        >
          Proporcione la información de los accionistas de su nuevo cliente como
          persona júridica.
        </p>
      </div>

      <RadioYesNo
        v-model="models.have_shareholder"
    ...
    <!-- Tabla principal -->
    <template v-if="models.have_shareholder">
      <section class="q-mt-lg">
        ...
        >
          ...
          <template #actions="{ row }">
            <Button
              v-if="action == 'view'"
              :leftIcon="defaultIconsLucide.eye"
              color="orange"
              :class-custom="'custom'"
              :outline="false"
              :flat="true"
            ...
            <Button
              v-if="action !== 'view'"
              :leftIcon="defaultIconsLucide.trash"
              color="orange"
              :class-custom="'custom'"
              :outline="false"
              :flat="true"
...
import SelectionModal from './ShareholderGenerator/SelectionModal.vue'
import Button from '@/components/common/Button/Button.vue'
import ShareholderGenerator from './ShareholderGenerator/ShareholderGenerator.vue'
import { useUtils } from '@/composables'
import useShareHolderForm from './ShareholderForm'
...
} = useShareHolderForm(props)

defineExpose({
  isRequiredForm: () =>
    !(
      models.value.have_shareholder === true &&
      tableProperties.rows.length === 0
    ),
  itHasTotalPercent: () => validateTotalPercent(),
  validateForm: () => formShareholder.value?.validate(),
})
</script>
