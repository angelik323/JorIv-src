...
import { IWarrantyFormData, ITtvOption, IOptions } from '@/interfaces/customs'
...
const useRepoWarrantyForm = (props: {
  action: ActionType
  ...
  const warrantyFormRef = ref<QForm | null>(null)
  ...
  const formData = ref<IWarrantyFormData>({
    rate: props.data?.rate ?? null,
    title_id: props.data?.title_id ?? null,
    issuer_id: props.data?.issuer_id ?? null,
    issuer_description: props.data?.issuer_description ?? '',
    isin_code: props.data?.isin_code ?? '',
    mnemonic: props.data?.mnemonic ?? '',
    paper_id: props.data?.paper_id ?? null,
    paper_description: props.data?.paper_description ?? '',
...
export default useRepoWarrantyForm
