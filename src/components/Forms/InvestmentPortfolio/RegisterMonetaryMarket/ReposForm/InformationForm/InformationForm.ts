...
const useRepoInformationForm = (props: {
  action: ActionType
  data?: IRepoInformationForm
}) => {
  ...
  const formData = ref<IRepoInformationForm>({
    ...
    repo: props.data?.repo ?? ('' as RepoPosition),
    repo_type: props.data?.repo_type ?? ('' as RepoTypes),
    ...
    days_base: props.data?.days_base ?? DaysBase.DAYS_360,
  ...
  const selectOptions = computed(() => ({
    ...
    compensation_systems: COMPENSATION_SYSTEMS as IOptionSelect[],
  }))
  ...
  const resetForm = () => {
    formData.value = {
      ...
      repo: '' as RepoPosition,
      repo_type: '' as RepoTypes,
      paper_id: null,
...
export default useRepoInformationForm
