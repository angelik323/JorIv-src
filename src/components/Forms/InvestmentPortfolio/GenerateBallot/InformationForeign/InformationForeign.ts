...
export const useInformationForeign = (
  ...
  const tableProps = ref<
    IBaseTableProps<IGenerateBallotFormDetail['titles'][number]>
  >({
    loading: false,
    wrapCells: true,
    columns: [
      ...
      {
        name: 'origin_currency_value',
        label: 'Valor moneda origen',
        field: (row) => row.origin_currency_value ?? '-',
        align: 'center',
      },
      {
        name: 'operation_origin_currency_value',
        label: 'Valor operación moneda origen',
        field: (row) => row.operation_origin_currency_value ?? '-',
        align: 'center',
      },
      {
        name: 'local_currency_transaction_value',
        label: 'Valor operación moneda local',
        field: (row) => row.local_currency_transaction_value ?? '-',
        align: 'center',
      },
      {
        name: 'resource_placement_date',
        label: 'Fecha colocación de recursos',
        field: (row) => row.resource_placement_date ?? '-',
        align: 'center',
      },
      {
        name: 'compliance_date',
        label: 'Fecha cumplimiento',
        field: (row) => row.compliance_date ?? '-',
        align: 'center',
      },
      {
        name: 'beneficiary',
        label: 'Beneficiario',
        field: (row) => row.beneficiary ?? '-',
        align: 'center',
      },
      {
        name: 'actions',
  ...
  const updateTable = () => {
    ...
    tablePropsForeign.value.rows = [...paginatedRows.value]
  ...
  const addOperationRow = () => {
    ...
    tableProps.value.rows.push({
      title_id: newRow.titles[0].title_id,
      operation_type: newRow.titles[0].operation_type,
      operation_value: newRow.titles[0].operation_value,
      beneficiary: newRow.titles[0].beneficiary,
      id: newRow.titles[0].id,
    })
    models.value.titles.push({
  ...
  const editItem = async (index: number) => {
    ...
    valueTypePayment.value = matchedMethod ?? null
  ...
  const addPaymentMethod = () => {
    ...
    const newTitleRow = {
      ...
      origin_currency_value: selectedTitle.origin_currency_value ?? '',
      operation_origin_currency_value:
        selectedTitle.operation_origin_currency_value ?? '',
      local_currency_transaction_value:
        selectedTitle.local_currency_transaction_value ?? '',
      resource_placement_date: selectedTitle.resource_placement_date ?? '',
      compliance_date: selectedTitle.compliance_date ?? '',
      currency: selectedTitle.currency ?? '',
  ...
  const preloadModalData = async () => {
    ...
    investment_portfolio_titles.value = data.titles.map(
      (item: {
        ...
        id: item.id,
        ...(item.origin_currency_value && {
          origin_currency_value: item.origin_currency_value,
        }),
        ...(item.operation_origin_currency_value && {
          operation_origin_currency_value: item.operation_origin_currency_value,
        }),
        ...(item.local_currency_transaction_value && {
          local_currency_transaction_value:
            item.local_currency_transaction_value,
        }),
        ...(item.resource_placement_date && {
