      ...
      >
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
          <GenericSelector
            ...
            @update:model-value="handlePaperTypeChange($event)"
          />
          <div v-else class="text-black-90">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
          <GenericSelector
            v-if="!isView"
            label="Mayor cuenta por cobrar"
            :default_value="models.higher_account_receivable"
            :manual_option="accounts_charts"
            map_options
            first_filter_option="label"
            second_filter_option="label"
            ...
            :required="false"
            :rules="[]"
            @update:model-value="models.higher_account_receivable = $event"
          />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Mayor cuenta por cobrar</p>
            <p class="text-weight-medium no-margin">
              {{ models.higher_account_receivable ?? 'No registrado' }}
            </p>
          </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
          <GenericSelector
            v-if="!isView"
            label="Auxiliar"
            :default_value="models.receivable_auxiliary"
            :manual_option="type_auxiliary"
            map_options
            first_filter_option="label"
            second_filter_option="label"
            auto_complete
            :required="models.higher_account_receivable !== null"
            :rules="models.higher_account_receivable !== null ? [
                (v: string) =>
                  useRules().is_required(v, 'El campo Auxiliar es requerido'),
              ] : []"
            @update:model-value="models.receivable_auxiliary = $event"
          />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Auxiliar</p>
            <p class="text-weight-medium no-margin">
              {{ models.receivable_auxiliary ?? 'No registrado' }}
            </p>
          </div>
        </div>
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
          <GenericSelector
            v-if="!isView"
            label="Mayor cuenta contrapartida"
            ...
            :rules="[
                (v: string) =>
                  useRules().is_required(v, 'El campo Mayor cuenta contrapartida es requerido'),
              ]"
            @update:model-value="models.counterparty_account = $event"
          />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Mayor cuenta contrapartida</p>
            <p class="text-weight-medium no-margin">
        ...
        <div class="col-xs-12 col-sm-12 col-md-3">
          <GenericSelector
            v-if="!isView"
            label="Centro de costos"
            :default_value="models.counterparty_cost_center"
            :manual_option="cost_center"
            map_options
            first_filter_option="label"
            second_filter_option="label"
            ...
            :required="false"
            :rules="[]"
            @update:model-value="models.counterparty_cost_center = $event"
          />
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">Centro de costos</p>
            <p class="text-weight-medium no-margin">
              {{ models.counterparty_cost_center ?? 'No registrado' }}
            </p>
          </div>
        </div>
...
import { nature_options } from '@/constants/resources'

import { IDefinitionAccountingParametersDetails } from '@/interfaces/customs'
import useDetailForm from './DetailForm'
