    ...
    <section aria-label="Sección de formulario de datos básicos">
      ...
      >
        ...
        <div class="col-12 col-md-3">
          <GenericInputComponent
            v-if="!isView"
            :default_value="formData.folio_number"
            label="Folio"
            placeholder="Inserte"
            required
            :rules="[
              (val: string) => useRules().is_required(val, 'El folio es requerido'),
              (val: string) => useRules().min_length(val, 5),
              (val: string) => useRules().max_length(val, 5),
              (val: string) => useRules().only_number(val),
            ]"
            @update:modelValue="formData.folio_number = $event"
          />
          <div class="text-black-90" v-else>
...
import useInformationForm from '@/components/Forms/InvestmentPortfolio/EquityOps/ETFForeignBuy/InformationForm/InformationForm'
