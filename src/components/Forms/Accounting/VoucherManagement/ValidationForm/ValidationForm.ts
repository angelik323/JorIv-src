...
const createDefaultModels = (): IVoucherManagementValidationForm => ({
  period_date: null,
  structure: null,
...
const useValidationForm = (
  _action: ActionType,
  _data: IVoucherManagementValidationForm | null
) => {
  const { openMainLoader } = useMainLoader()
  const { defaultIconsLucide } = useUtils()

  const basicDataFormRef = ref()
  const models = ref<IVoucherManagementValidationForm>(createDefaultModels())
  ...
  const onChangeStructure = async (event: number) => {
    ...
    descriptionsFields.value.accounting_structure_purpose = ''
