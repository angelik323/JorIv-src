...
const useInformationForm = (props: { action: ActionType; id?: number }) => {
  ...
  const listVoucherLines = async () => {
    ...
    if (result) {
      amountsTableProps.value.rows = result.rows
  ...
  const setAccountValue = ($event: number, row: IVoucherAmount) => {
    ...
    const voucherData = accounting_receipt.value.voucher_data.map(
      (item: IVoucherAmount) => {
        const itemData: IVoucherAmount = {
          ...
          foreign_currency:
            Number(item.foreign_currency) == 0 ? '' : item.foreign_currency,
        }
  ...
  onMounted(async () => {
    if (props.action !== 'create' && props.id) {
      ...
      third_parties_label.value = third_parties_label.value.map(
      ...
      const structures =
        accounting_receipt.value.business_trust?.account
      ...
      if (structures && structures.length > 0) {
        ...
        structureCode = firstStructure?.account_structure?.code
        structurePurpose = firstStructure?.account_structure?.purpose
        accounting_receipt.value.accounting_structure_id =
          firstStructure?.account_structure?.id
        structureType = firstStructure?.type ?? 'Original'
      } else {
        const fallback =
          accounting_receipt.value.business_trust.account?.account_structure
        structureCode = fallback?.code
        structurePurpose = fallback?.purpose
        accounting_receipt.value.accounting_structure_id = fallback?.id
