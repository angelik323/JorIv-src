...
const useRevertHomologationForm = () => {
  const {
  ...
  const processedHomologationsTableProps = ref({
    title: 'Listado para deshacer homologación',
    loading: false,
    columns: [
      {
        name: 'id',
  ...
  const searchVouchers = async () => {
    if (await informationForm.value.validate()) {
      ...
      filterable_vouchers_list.value = []
      _searchFilterableVouchers(formatParamsCustom(params))
    }
  }
  ...
  const homologate = async () => {
    if (await informationForm.value.validate()) {
      ...
      _cleanHomologationProcessData()
      _selectiveHomologation(payload)
    }
  }
...
export default useRevertHomologationForm
