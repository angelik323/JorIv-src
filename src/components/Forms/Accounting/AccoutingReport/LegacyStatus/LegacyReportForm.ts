...
const useLegacyReportForm = (
  ...
  const legacy = useAccoutingReportStore(
    'v1',
    'legacyStatus'
  ...
  const models = ref<ILegacyReportModel>({
    report_template_id: '',
    level: '',
    business_id: 0,
  ...
  const handleContinue = async () => {
    ...
    const filters = {
      ...
      'filter[period_from]': models.value.from_period,
      'filter[period_to]': models.value.to_period,
      'filter[comparative_from]': models.value.from_period,
      'filter[comparative_to]': models.value.to_period,
      report_template_id: models.value.report_template_id,
    }

    const result = await legacy._getLegacyStatusBalance(filters)
  ...
  const isFormValid = computed(() => {
    ...
    return !!m.business_id && !!m.level && !!m.from_period && !!m.to_period
  })
...
export default useLegacyReportForm
