...
const useQuarterlyBalanceForm = (
  ...
  const validateQuarterlyMonth = (dateString: string): string | true => {
    ...
    if (!QUARTERLY_MONTHS.includes(month)) {
      return 'Solo se permiten períodos trimestrales (03, 06, 09, 12)'
    }
  ...
  const handleDateSelection = (date: string) => {
    ...
    models.value.period = date
    if (!QUARTERLY_MONTHS.includes(month)) {
      models.value.last_period = ''
      return
    }
    models.value.last_period = `${prevYear}-${String(prevMonth).padStart(
      2,
      '0'
    )}`
  }
  ...
  const handleContinue = async () => {
    ...
    const result = await quarterlyBalance._getQuarterlyStatementBalance(filters)
...
export default useQuarterlyBalanceForm
