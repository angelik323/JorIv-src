...
const useGeneralResultsForm = (
  ...
  const models = ref<IGeneralReportModel>({
    report_template_id: '',
    level: '',
    business_id: 0,
  ...
  const handleContinue = async () => {
    ...
    const filters = {
      ...
      'filter[period]': models.value.from_period,
      level: models.value.level,
      'filter[last_period]': models.value.to_period,
      report_template_id: models.value.report_template_id,
    }

    const result = await generalResults._getGeneralResultsBalance(filters)
    openMainLoader(false)
    if (result) emits('enable-preview-tab')
  }
  ...
  const toPrevYearSameMonth = (p?: string) => {
  ...
  watch(
    ...
    (val) => {
      models.value.to_period = toPrevYearSameMonth(val)
    },
    { immediate: true }
  )
...
export default useGeneralResultsForm
