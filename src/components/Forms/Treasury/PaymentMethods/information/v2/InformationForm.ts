...
const useInformationForm = (props: {
  ...
  const { _setDataInformationForm } = usePaymentMethodsStore('v2')
  ...
  const setFormEdit = async () => {
    ...
    if (data) {
      ...
      models.value.message_check = data?.message_check ?? null
      models.value.exchange_days = data?.exchange_days ?? null
    }
  }
  ...
  const clearForm = () => {
    ...
    models.value.message_check = null
    models.value.request_bank_withdrawal = false
    models.value.exchange_days = null
  }
    ...
    () => {
      if (isEmptyOrZero(models.value)) {
        ...
        _setDataInformationForm({
          ...
          message_check: models.value.message_check ?? null,
          request_bank_withdrawal:
            models.value.request_bank_withdrawal ?? false,
          status_id: models.value.status_id ?? 1,
          exchange_days: models.value.exchange_days ?? null,
        })
      }
    },
    { deep: true }
