    ...
    <section v-if="['create', 'view', 'edit'].includes(action)">
      <div class="row q-col-gutter-x-lg q-col-gutter-y-md">
        ...
        <div class="col-12 col-md-4">
          <GenericSelectorComponent
            v-if="['create', 'edit'].includes(action)"
            label="Códigos movimientos de tesorería"
            :default_value="models.real_estate_movement"
            :manual_option="treasury_movement_codes"
            :auto_complete="true"
            :map_options="true"
            :disabled="!isTreasuryMovementEnabled"
            :required="isTreasuryMovementEnabled"
            :rules="[
              (val: string) =>
                isTreasuryMovementEnabled
                  ? is_required(val, 'El código movimiento de tesorería es requerido')
                  : true,
            ]"
          ...
          <div v-else class="text-black-90">
            <p class="text-weight-bold no-margin">
              Códigos movimientos de tesorería
            </p>
            <p class="text-weight-medium no-margin">
...
// Logic view
import useInformationForm from '@/components/Forms/Fics/MovementCodes/v2/Information/InformationForm'
