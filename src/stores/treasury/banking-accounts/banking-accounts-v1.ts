...
export const useBankingAccountsV1 = defineStore('banking-accounts-v1', {
  state: () => ({
  ...
  actions: {
    async _getBankingAccountsList(params: string) {
      ...
      await executeApi()
        .get(`${URL_PATH_TREASURIES}/bank-accounts?paginate=1${params}`)
        .then((response) => {
    ...
    async _getByIdBankingAccounts(id: number) {
      ...
      await executeApi()
        .get(`${URL_PATH_TREASURIES}/bank-accounts/${id}`)
        .then((response) => {
          if (response.data.success) {
    ...
    async _isValidDataBasicForm(payload: object) {
      ...
      await executeApi()
        .post(
          `${URL_PATH_TREASURIES}/bank-accounts/validate-basic-data`,
          payload
        )
        .then((response) => {
          success = response.data.success
    ...
    async _isValidDataBasicEditForm(payload: object) {
      let success = false
      await executeApi()
        .post(
          `${URL_PATH_TREASURIES}/bank-accounts/validate-basic-data-update`,
          payload
        )
        .then((response) => {
          success = response.data.success
