...
describe('useRemoteAuthorizationStoreV1', () => {
  beforeEach(() => {
  ...
  it('fetches list and updates state on success', async () => {
    const store = useRemoteAuthorizationStoreV1()
    ...
    expect(store.authorization_list).toEqual([
      { id: 10, code: 'A-001', status: 'pending' },
    ])
    expect(store.authorization_pages).toEqual({
      currentPage: 2,
      lastPage: 5,
    })
