...
describe('useDividendLocals', () => {
  ...
  it('should fetch operational etf list and update state on success', async () => {
    // Arrange
    const store = useDividendForeignStoreV1()
    const mockResponse = {
      data: {
        success: true,
        data: {
    ...
    // Act
    await store._getDividendForeignList(params)

    // Assert
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.dividend_foreign_list).toEqual(mockResponse.data.data.data)
  })
  ...
  it('should handle error when fetching operational etf list', async () => {
    ...
    // Assert
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.dividend_foreign_list).toEqual([])
  })
  ...
  it('should handle response.data as undefined when fetching the operational etf list', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.dividend_foreign_list).toEqual([])
    expect(store.dividend_foreign_pages.currentPage).toBe(0)
    expect(store.dividend_foreign_pages.lastPage).toBe(0)
  })
  ...
  it('should create an operational etf successfully', async () => {
    ...
    // Act
    const result = await store._createDividendForeign(payload)
  ...
  it('should fetch an operational etf by ID', async () => {
    ...
    const id = 1
    // Act
    const response = await store._getDividendForeign(id)
    // Assert
  ...
  it('should update an operational etf successfully', async () => {
    ...
    // Act
    const result = await store._updateDividendForeign(id, payload)
    // Assert
  ...
  it('should toggle operational etf status successfully', async () => {
    ...
    const id = 1
    // Act
    const result = await store._updateDividendForeignStatus(id)
    // Assert
