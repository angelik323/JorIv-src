...
describe('useRegisterDividendsForeignCurrencyStoreV1', () => {
  const BASE_URL = `${URL_PATH_INVESTMENT_PORTFOLIO}/foreign-currency-shar`
  ...
  it('lists data correctly on _listAction', async () => {
    const store = useRegisterDividendsForeignCurrencyStoreV1()
    const mockResponse = {
      data: {
        success: true,
    ...
    expect(store.register_dividends_foreign_currency_list).toEqual([{ id: 1 }])
    expect(store.register_dividends_foreign_currency_pages.currentPage).toBe(2)
    expect(store.register_dividends_foreign_currency_pages.lastPage).toBe(3)
  })
  ...
  it('handles error in _listAction', async () => {
    ...
    expect(store.register_dividends_foreign_currency_list).toEqual([])
  })
  ...
  it('clears data with _clearData', () => {
    ...
    store.register_dividends_foreign_currency_list = [
      { emitter_id: 123 },
    ] as Partial<IEmitterDividend>[] as IEmitterDividend[]
    store.register_dividends_foreign_currency_pages.currentPage = 4
    store.register_dividends_foreign_currency_pages.lastPage = 10

    store._clearData()

    expect(store.register_dividends_foreign_currency_list).toEqual([])
    expect(store.register_dividends_foreign_currency_pages).toEqual({
      currentPage: 1,
      lastPage: 1,
    })
  })
