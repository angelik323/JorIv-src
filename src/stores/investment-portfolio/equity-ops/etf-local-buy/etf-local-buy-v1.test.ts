...
describe('useLocalPurchaseExchangeTradedFundStoreV1', () => {
  const BASE_URL = `${URL_PATH_INVESTMENT_PORTFOLIO}/local-purchase-exchange-traded-fund`
  ...
  it('returns data correctly in _showAction', async () => {
    const store = useLocalPurchaseExchangeTradedFundStoreV1()
    const mockResponse = {
      data: {
        success: true,
  ...
  it('creates data correctly in _createAction', async () => {
    ...
    const payload = {
      ...
      commission_value: 20,
      value_unit_buy: '12.50',
      value_compliance: '1250.00',
      seller_id: 5,
    } as IEtfLocalBuyOperation
  ...
  it('returns false on error in _createAction', async () => {
    ...
    const result = await store._createAction({} as IEtfLocalBuyOperation)
