import { useForeignPurchaseExchangeTradedFundStoreV1 } from './etf-foreign-buy-v1'
...
describe('useForeignPurchaseExchangeTradedFundStoreV1', () => {
  const BASE_URL = `${URL_PATH_INVESTMENT_PORTFOLIO}/foreign-purchase-exchange-traded-fund`
  ...
  it('returns data correctly in _showAction', async () => {
    const store = useForeignPurchaseExchangeTradedFundStoreV1()
    const mockResponse = {
      data: {
        success: true,
  ...
  it('returns false on error in _createAction', async () => {
    ...
    const result = await store._createAction({} as IEtfForeignBuyOperation)
