...
describe('useOperationalETFs', () => {
  ...
  it('should fetch operational etf list and update state on success', async () => {
    // Arrange
    const store = useOperationalETFStoreV1()
    const mockResponse = {
      data: {
        success: true,
        data: {
    ...
    // Act
    await store._getOperationalETFList(params)

    // Assert
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.operational_etf_list).toEqual(mockResponse.data.data.data)
  })
  ...
  it('should handle error when fetching operational etf list', async () => {
    ...
    // Assert
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.operational_etf_list).toEqual([])
  })
  ...
  it('should handle response.data as undefined when fetching the operational etf list', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith(`${URL_PATH}/list?paginate=1${params}`)
    expect(store.operational_etf_list).toEqual([])
    expect(store.operational_etf_pages.currentPage).toBe(0)
    expect(store.operational_etf_pages.lastPage).toBe(0)
  })
  ...
  it('should create an operational etf successfully', async () => {
    ...
    // Act
    const result = await store._createOperationalETF(payload)
  ...
  it('should fetch an operational etf by ID', async () => {
    ...
    const id = 1
    // Act
    const response = await store._getOperationalETF(id)
    // Assert
  ...
  it('should update an operational etf successfully', async () => {
    ...
    // Act
    const result = await store._updateOperationalETF(id, payload)
    // Assert
  ...
  it('should toggle operational etf status successfully', async () => {
    ...
    const id = 1
    // Act
    const result = await store._updateOperationalETFStatus(id)
    // Assert
