...
export const usePortfolioClassificationStoreV1 = defineStore(
  'portfolio-classification-store-v1',
  {
    state: () => ({
      version: 'v1',
    ...
    actions: {
      ...
      async _changeStatus(portfolioId: number) {
        let success = false

        await executeApi()
          .patch(
            `${URL_PORTFOLIO_CLASSIFICATIONS}/${portfolioId}/toggle-status`
          )
          .then((response) => {
            const { message } = response.data
            success = response.data?.success ?? false
