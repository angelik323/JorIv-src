...
export const usePaymentMethodsStoreV1 = defineStore('payment-methods-v1', {
  state: () => ({
    version: 'v1',
    payment_methods_list: [] as IPaymentMethod[],
    payment_methods_pages: {
      currentPage: 0,
      lastPage: 0,
    },
    data_information_form: null as IPaymentMethod | null,
    payment_methods_request: null as IPaymentMethod | null,
  }),
  actions: {
    ...
    async _createPaymentMethod(data: IPaymentMethod): Promise<boolean> {
      let success = false
    ...
    async _updatePaymentMethod(data: IPaymentMethod, id: number) {
      let success = false
    ...
    _setDataInformationForm(data_to_set: IPaymentMethod | null) {
      this.data_information_form = data_to_set ? { ...data_to_set } : null
    },

    _clearData() {
      this.payment_methods_list = []
      this.payment_methods_request
