...
export const useDiaryBookReportV1 = defineStore('diary-book-report-store-v1', {
  state: () => ({
    version: 'v1',
    diary_book_list: [] as IAccountingReport[],
    diary_book_pages: {
      currentPage: 0,
      lastPage: 0,
    },
  ...
  actions: {
    _getDiaryBookBalance(params: Record<string, string | number | boolean>) {
      ...
      const finalUrl = `${URL_PATH_ACCOUNTING}/diary-book-report?${query}&paginate=1`
      ...
      return executeApi()
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            this.diary_book_list = response.data.data.data.data ?? []

            this.diary_book_pages.currentPage =
              response.data.data.data.current_page ?? 1
            this.diary_book_pages.lastPage =
              response.data.data.data.last_page ?? 1
            this.report_excel_url = replaceDevWithAccounting(
              response.data.data.data.reportables?.report_excel_url
            )

            this.report_pdf_url = replaceDevWithAccounting(
              response.data.data.data.reportables?.report_pdf_url
            )
