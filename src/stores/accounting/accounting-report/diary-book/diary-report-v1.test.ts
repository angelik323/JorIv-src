...
import { useDiaryBookReportV1 } from './diary-report-v1'
...
describe('useDiaryBookReportV1', () => {
  ...
  it('_getDiaryBookBalance - success', async () => {
    mockGet.mockResolvedValueOnce({ data: mockData })

    const store = useDiaryBookReportV1()
    const params = { 'filter[business_trust_id]': 1 }

    const result = await store._getDiaryBookBalance(params)

    expect(mockGet).toHaveBeenCalledWith(
      expect.stringContaining('diary-book-report?')
    )
  ...
  it('_getDiaryBookBalance - error en response', async () => {
    mockGet.mockResolvedValueOnce({
      data: { success: false, message: 'Error de API' },
    })

    const store = useDiaryBookReportV1()
    const result = await store._getDiaryBookBalance({})
  ...
  it('_getDiaryBookBalance - exception', async () => {
