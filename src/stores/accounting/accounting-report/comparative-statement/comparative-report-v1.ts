...
export const useComparativeReportV1 = defineStore(
  'comparative-report-store-v1',
  {
    state: () => ({
      version: 'v1',
      comparative_financial_list: [] as IAccountingReport[],
      comparative_financial_pages: {
        currentPage: 0,
        lastPage: 0,
      },
    ...
    actions: {
      _getComparativeStatementBalance(params: Record<string, any>) {
        ...
        const finalUrl = `${URL_PATH_ACCOUNTING}/comparative-financial-statement-report/generate?${query}&paginate=1`
        ...
        return executeApi()
          ...
          .then((response) => {
            if (response.data.success) {
              ...
              this.comparative_financial_list = response.data.data.data ?? []

              this.comparative_financial_pages.currentPage =
                response.data.data.current_page ?? 1
              this.comparative_financial_pages.lastPage =
