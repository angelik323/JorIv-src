...
export const useDailyBalanceV1 = defineStore('daily-report-store-v1', {
  state: () => ({
    version: 'v1',
    daily_balance_list: [] as IAccountingReport[],
    daily_balance_pages: {
      currentPage: 0,
      lastPage: 0,
    },
  ...
  actions: {
    _getDailyBalance(
      ...
      const finalUrl = `${URL_PATH_ACCOUNTING}/account-general-balance/generate?${query}&paginate=1`
      ...
      return executeApi()
        ...
        .then((response) => {
          if (response.data.success) {
            ...
            this.daily_balance_list = response.data.data.data ?? []

            this.daily_balance_pages.currentPage =
              response.data.data.current_page ?? 1
            this.daily_balance_pages.lastPage =
              response.data.data.last_page ?? 1

            const p = response.data.data as Record<string, unknown>
