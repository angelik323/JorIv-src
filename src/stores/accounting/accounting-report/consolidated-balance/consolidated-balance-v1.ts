...
export const useBalanceReportV1 = defineStore(
  'consolidated-balance-report-store-v1',
  {
    state: () => ({
      version: 'v1',
      consolidated_balance_list: [] as IAccountingReport[],
      consolidated_balance_pages: {
        currentPage: 0,
        lastPage: 0,
      },
    ...
    actions: {
      _getConsolidatedBalanceBalance(params: Record<string, unknown>) {
        const { showAlert } = useAlert()
        const { showCatchError } = useShowError()

        const toQueryParts = (key: string, val: unknown): string[] => {
          ...
          if (Array.isArray(val)) {
            const parts: string[] = []
            for (const item of val) parts.push(...toQueryParts(key, item))
            return parts
          }
          return []
        }
        ...
        const query = Object.entries(params)
          .flatMap(([k, v]) => toQueryParts(k, v))
          .join('&')
        const finalUrl = `${URL_PATH_ACCOUNTING}/consolidated-balance-report/generate?${query}&paginate=1`
        ...
        return executeApi()
          ...
          .then((response) => {
            if (response.data.success) {
              ...
              this.consolidated_balance_list = response.data.data?.data ?? []
              this.consolidated_balance_pages.currentPage =
                response.data.data?.current_page ?? 1
              this.consolidated_balance_pages.lastPage =
          ...
          .then(async (response) => {
            ...
            if (ct.includes('application/json')) {
              ...
              try {
                ...
                a.download = 'Balance-Comparativo.xlsx'
