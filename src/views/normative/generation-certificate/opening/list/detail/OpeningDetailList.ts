...
export const useGenerationOpeningCertificateList = () => {
  const {
  ...
  const headerProperties = {
    title: 'Generación certificados de apertura',
    breadcrumbs: [
      ...headerPropsDefault.breadcrumbs,

      {
        label: 'Generación certificado apertura',
        route: 'OpeningGroupList',
      },
    ],
  }
  ...
  >({
    title: 'Listado de generación de certificados de Apertura',
    loading: false,
    wrapCells: true,
    columns: [
      {
  ...
  const goToView = (row: IGenerationCertificateDetailListItem) => {
    ...
    _setRouterList('OpeningGroupList')
    _setUrl(row.url)
  }
