...
export const useGenerationRetentionCertificateList = () => {
  const {
  ...
  const headerProperties = {
    title: 'Generación certificados de retención',
    breadcrumbs: [
      ...headerPropsDefault.breadcrumbs,

      {
        label: 'Generación certificado retención',
        route: 'RetentionGroupList',
      },
    ],
  }
  ...
  >({
    title: 'Listado de generación de certificados de retención',
    loading: false,
    wrapCells: true,
    columns: [
      {
  ...
  const goToView = (row: IGenerationCertificateDetailListItem) => {
    ...
    _setRouterList('RetentionGroupList')
    _setUrl(row.url)
  }
