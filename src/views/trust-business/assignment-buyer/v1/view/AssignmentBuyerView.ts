...
const useAssignmentBuyerView = () => {
  ...
  const { _clearData, _getByIdAction } = useAssignmentBuyerStore('v1')

  const { data_response, selectedThirdId, data_tables } = storeToRefs(
  ...
  const goToList = () => {
    router.push({ name: 'AssignmentBuyerList' })
  }
...
export default useAssignmentBuyerView
