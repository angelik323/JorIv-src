...
const useDiscontinuancesAuthorize = () => {
  // imports
  ...
  const setAlertModalDescription = () => {
    return `¿Está seguro que desea ${
      alertModalConfig.value.action ? 'autorizar' : 'rechazar'
    } el negocio?`
  }
  ...
  watch(
    ...
    async () => {
      ...
      observation.value = data.observations ?? ''
    },
    { deep: true }
  )
...
export default useDiscontinuancesAuthorize
