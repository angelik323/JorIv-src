...
const useRecordTransfersEdit = () => {
  ...
  const headerProps = {
    title: 'Editar registro de cesión',
    breadcrumbs: [
      {
        label: 'Inicio',
        route: 'HomeView',
  ...
  const onSubmit = async () => {
    ...
    const responseUpdate = await _updateRecordTransfers(
      recordTransfersId ?? 0,
      payload
    )
    if (responseUpdate && data_documents_form.value.length) {
      await _updateRecordTransfers(recordTransfersId ?? 0, payload)
      await _deleteFilesAction(recordTransfersId ?? 0)
...
export default useRecordTransfersEdit
