...
import { dispersion_options } from '@/constants/resources'
import {
  IFieldFilters,
...
const useDetailDispersionGroupList = () => {
  ...
  const {
    ...
    _createAction,
    _listDetailAction,
    _listBreakdownAction,
  } = detailDispersionGroupStore

  const selectedBreakdownRow = ref<IDispersionBreakdownGroupResponse | null>(
    null
  )
  ...
  const tablePropertiesDetail = ref({
    ...
    columns: [
      ...
      {
        name: 'method_payment_code',
        align: 'left',
        label: 'Código de forma de pago',
        field: 'method_payment_code',
        sortable: true,
      },
      {
    ...
    rows: [] as IDispersionDetailGroupResponse[],
  })
  ...
  const tablePropertiesBreakdown = ref({
    ...
    rows: [] as IDispersionBreakdownGroupResponse[],
  })
  ...
  const loadData = async (filters: string, type: 'detail' | 'breakdown') => {
    openMainLoader(true)
    if (type === 'detail') await _listDetailAction(filters)
    else {
      selectedBreakdownRow.value = null
      selectedRowIdBreakdown.value = []
      await _listBreakdownAction(filters)
    }
  ...
  const handleSubmitGroupDispersion = async () => {
    ...
    const payload: IDetailDispersionGroupRequest[] =
      selectedRowIdBreakdown.value.map((row) => {
        const {
          ...
          group = selectedGroup.value ?? false,
        } = row as Partial<IDetailDispersionGroupRequest>
  ...
  const handleChangeFilters = () => {
    if (selectedDetailRow.value) {
      const filters = {
        'filter[option]': selectedRadioBreakdown.value ?? '',
  ...
  watch(selectedRowIdDetail, (newVal) => {
    if (newVal !== null) {
      ...
      if (selectedDetailRow.value) {
        const filters = {
          'filter[option]': selectedRadioBreakdown.value ?? '',
          'filter[method_payment_id]':
            selectedDetailRow.value.method_payment_id,
          'filter[bank_id]': selectedDetailRow.value.bank_id,
          'filter[bank_account_id]': selectedDetailRow.value.bank_account_id,
          'filter[office_id]': Number(filtersFormat.value['filter[office]']),
          'filter[business_id]': selectedDetailRow.value.business_id,
  ...
  watch(selectedRowIdBreakdown, async (newVal) => {
    if (newVal && newVal.length > 0) {
      ...
      const data = await _showAction(selectedId)
      if (data) {
  ...
  watch(
    ...
    () =>
      (tablePropertiesBreakdown.value.rows = dispersion_breakdown_list.value)
  )

  const hasSelectedRows = computed(
    () => selectedRowIdBreakdown.value.length > 0
  )
