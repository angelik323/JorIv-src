...
const useRemoteTreasuryAuthorizationList = () => {
  ...
  const { _getListAction, _cleanDataList } = raStore
  const { authorization_list, authorization_pages } = storeToRefs(raStore)

  const { remote_authorization_statuses, remote_authorization_modules } =
    storeToRefs(useTreasuryResourceStore('v1'))

  const { _getResources } = useResourceManagerStore('v1')

  const keysV2 = {
    treasury: ['remote_authorization_statuses', 'remote_authorization_modules'],
  }
  ...
  const tableProps = ref<{
    ...
    columns: [
      ...
      },
      {
        ...
        label: 'Autorizable',
        align: 'left',
        field: (row) => row.authorizable?.description ?? '—',
      },
  ...
  watch(
    ...
    () => {
      tableProps.value.rows = [
        ...authorization_list.value.map((row) => ({
          ...row,
          disabled:
      ...
      const { currentPage, lastPage } = authorization_pages.value
