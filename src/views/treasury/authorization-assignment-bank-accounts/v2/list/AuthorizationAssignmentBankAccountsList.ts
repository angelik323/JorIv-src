...
export const useAuthorizationAssignmentBankAccountsList = () => {
  ...
  const {
    ...
    _hasErrors,
    _clearData,
  } = useAuthorizationAssignmentBankAccountsStore('v2')
  ...
  const selectedRows = ref<IAuthorizationAssignmentBankAccountsItem[]>([])

  const selectedStates = computed(() => ({
    hasError: selectedRows.value.some((row) => row.state === 30),
  }))

  const canShowErrors = computed(() => {
    return selectedStates.value.hasError || _hasErrors()
  })
  ...
  } = storeToRefs(useAuthorizationAssignmentBankAccountsStore('v2'))
  ...
  const toTransferTable = () => {
    authorization_assignment_bank_accounts_list.value =
      authorization_assignment_bank_accounts_list.value.map((item) => ({
        ...item,
        selected: false,
        uniqueKey: `${item.parent_id || 0}-${item.id}`,
  ...
  const makeDataRequestApprove = () => {
    const groupedByParentId = selectedRows.value.reduce((groups, row) => {
  ...
  const handleActions = async () => {
    ...
    if (currentAction.value === 'autorizar') {
      ...
      const success = await _authorizationAssignmentBankAccountsList(
        payloads as never
      )
  ...
  const handleOptions = async (option: string) => {
    ...
    switch (option) {
      case 'errores':
        const errorParentIds = [
          ...new Set(
            selectedRows.value
              .filter((row) => row.state === 30)
