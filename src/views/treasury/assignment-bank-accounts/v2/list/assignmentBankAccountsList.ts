...
const useAssignmentBankAccountsList = () => {
  ...
  } = useAssignmentBankAccountsStore('v2')

  const { assignment_bank_accounts_list, assignment_bank_accounts } =
    storeToRefs(useAssignmentBankAccountsStore('v2'))
  ...
  const handleFilter = async ($filters: {
    ...
    'filter[grantor_date]': string
    'filter[account_bank]': string
  }) => {
    filtersFormat.value = {
      ...$filters,
    }
  ...
  const listAction = async (filters: string = '') => {
    try {
      ...
      await _getAssignmentBankAccountsList(filters)

      const list = assignment_bank_accounts_list.value || []
  ...
  const onFilterChange = (filters: Record<string, string | number | null>) => {
    ...
    const selectedValue = filters['filter[business_id]']
  ...
  const makeDataRequest = (): IAssignmentBankAccountsPayload => {
    ...
    const businessTrustId = dataForm['filter[business_id]']
