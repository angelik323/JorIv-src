...
// Interfaces
import {
  ICommissionParametersForm,
  ICommissionParametersList,
} from '@/interfaces/customs/treasury/CommissionParameters'
...
const useAccountingParametersCommissionsEdit = () => {
  const { _getResources, _resetKeys } = useResourceManagerStore('v1')
  const { _getByIdCollectionAccountingBlocks } =
    useCollectionAccountingBlocksStore('v1')

  const {
    _getByIdCommissionAccountingParameters,
    _editCommissionAccountingParameters,
    _clearData,
  } = useCommissionParametersStore('v1')
  const { commission_accounting_parameters_request } = storeToRefs(
    useCommissionParametersStore('v1')
  )
  ...
  const commissionAccountingParamId = +route.params.idParam

  const { openMainLoader } = useMainLoader()
  const { cleanEmptyFields } = useUtils()

  const keys = {
    treasury: ['counter_auxiliary_type', 'third_parties'],
    budget: ['code_movements', 'budget_document_types'],
  }
  ...
  const makeBaseInfoRequest = (data: ICommissionParametersForm | null) => {
    ...
    const request: Partial<ICommissionParametersForm> = {
      accounting_blocks_collection_id:
        data.accounting_blocks_collection_id ?? null,
  ...
  const onSubmit = async () => {
    ...
    const success = await _editCommissionAccountingParameters(
      payload,
  ...
  onMounted(async () => {
    await _getByIdCommissionAccountingParameters(commissionAccountingParamId)
  })
  ...
  onBeforeMount(async () => {
    ...
    await _getResources({
      budget: ['budget_item_codes&filter[type]=Operativo'],
    })
    await _getResources({
      budget: ['areas_resposabilities_codes&filter[type]=Operativo'],
    })
    await _getResources({
      budget: ['budget_resource_codes&filter[type]=Operativo'],
    })
    openMainLoader(false)
  })
  ...
  onBeforeUnmount(() => {
    _resetKeys(keys)
    _resetKeys({
      treasury: ['accounting_account_contrapart', 'operational_cost_centers', 'cash_flow_structures'],
    })
    _resetKeys({
      budget: [
        'budget_item_codes',
  ...
  const setFormEdit = (data: ICommissionParametersList) => {
    data_information_form.value = {
      ...
      contra_account_chart_id: data.contra_account_chart?.id ?? null,
      contra_cost_center_id: data.contra_cost_center?.id ?? null,
      contra_aux_type: data.contra_aux_type ?? null,
      contra_third_party_id: data.contra_third_party?.id ?? null,
      contra_cash_flow_structure_id: data.contra_cash_flow_structure?.id ?? null,
  ...
  watch(
    () => commission_accounting_parameters_request.value,
    (val) => {
      if (!val) return
      setFormEdit(val)
    }
...
export default useAccountingParametersCommissionsEdit
