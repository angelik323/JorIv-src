...
const { business_bank_accounts, checkbooks, checks } = storeToRefs(
...
const useExpenseChecksList = () => {
  ...
  const openExpenseChecksModal = async (infoModal: {
    id: number
    checks: number
  }) => {
    openMainLoader(true)
    ...
    const checksLabel =
      String(infoModal.checks)
        .replace(/^Cheque\s*/i, '')
        .trim() || String(infoModal.checks)
    alertModalConfig.value.title = `Información del cheque #${checksLabel}`
    alertModalConfig.value.entityId = infoModal.id
    alertModalConfig.value.description = `Impresión de cheques con egreso.`
  ...
  const handlerShowViewerFile = async (obligationId: number) => {
    ...
    const result = await _loadPrintCheckPDF(66, obligationId)
    await alertExpenseChecksModalRef.value.closeModal()
    openMainLoader(false)
    if (result) {
      await updatePage(1)
  ...
  const loadPrintChecks = async (expenseChecks: IExpenseCheckItem[]) => {
    ...
    await _loadPrintChecks(66, checkIds)
    await updatePage(1)
  ...
  watch(
    () => expenseChecksList.value,
    () => {
      tableProps.value.rows = getExpenseChecksList.value
    }
  )
...
export default useExpenseChecksList
