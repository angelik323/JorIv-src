...
const useLegalEntity = () => {
  ...
  const { _createLegalClient, _clearDataLegal } = useClientsStore('v1')
  ...
  const makeFinancialInfoRequest = (data: ILegalClientCorporative) => {
    return removeEmpty({
      ...
      other_non_operational_income_concept:
        data.item_other_monthly_income_corporative?.toString() ?? null,
  ...
  const onSubmit = async () => {
    if (await validateForms()) {
      ...
      if (await _createLegalClient(payload)) {
