...
export default defineComponent({
  ...
  setup() {
    ...
    const { _getByIdNaturalClient, _updateNaturalClient, _clearDataNatural } =
      useClientsStore('v1')
    ...
    const makeDataRequest = () => {
      ...
      const extractPepRelativeInfo = (key: string) =>
        (
          data_pep_form.value?.pep_info_relative as unknown as Record<
            string,
            any
          >
        )?.[key] ?? null
      ...
      const dataPayload: Record<string, any> = {
        ...
        contacts: [
          {
            contact_type: 'email',
            contact_value: extractContact(0),
            is_main: 0,
          },
          {
            contact_type: 'phone',
            contact_value: extractContact(1),
            is_main: 0,
          },
          {
            contact_type: 'mobile',
        ...
        tax_info: {
          ...
          is_withholding_subject:
            Number(extractTaxInfo('withholding_tax')) || 0,
    ...
    const onSubmit = async () => {
      if (await validateForms()) {
        ...
        if (await _updateNaturalClient(payload, naturalClientId)) {
