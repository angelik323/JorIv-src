...
const useMarginCallView = () => {
  ...
  const marginCallId = Number(router.currentRoute.value.params.id)

  const headerProperties = {
    ...headerPropsDefault.value,
    title: 'Ver llamado al margen',
    breadcrumbs: [
      ...headerPropsDefault.value.breadcrumbs,
      {
        label: 'Ver llamado al margen',
      },
    ],
  }
  ...
  onMounted(async () => {
    openMainLoader(true)
    const data = await _getGuaranteeOperationById(marginCallId)
    if (data) {
...
export default useMarginCallView
