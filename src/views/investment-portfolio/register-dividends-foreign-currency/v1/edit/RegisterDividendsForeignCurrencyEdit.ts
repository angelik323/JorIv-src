...
const useRegisterDividendsForeignCurrencyEdit = () => {
  const { openMainLoader } = useMainLoader()
  const { defaultIconsLucide } = useUtils()
  const route = useRoute()

  const { _showAction, _updateAction } =
    useRegisterDividendsForeignCurrencyStore('v1')
  ...
  const headerProperties = {
    title: 'Editar registro dividendos acciones moneda extranjera',
    breadcrumbs: [
      {
        label: 'Inicio',
        route: 'HomeView',
      ...
      {
        label: 'Editar',
        route: 'RegisterDividendsForeignCurrencyEdit',
      },
      {
        label: id,
      },
...
export default useRegisterDividendsForeignCurrencyEdit
