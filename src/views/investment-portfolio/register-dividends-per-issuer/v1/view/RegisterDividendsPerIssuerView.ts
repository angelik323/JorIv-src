...
import { useRegisterDividendsPerIssuerStore } from '@/stores'

const useRegisterDividendsPerIssuerView = () => {
  const { openMainLoader } = useMainLoader()
  const { defaultIconsLucide } = useUtils()
  const route = useRoute()

  const { _showAction } = useRegisterDividendsPerIssuerStore('v1')
  ...
  const headerProperties = {
    title: 'Ver registro de dividendos por emisor acciones',
    breadcrumbs: [
      {
        label: 'Inicio',
        route: 'HomeView',
      ...
      {
        label: 'Ver',
        route: 'RegisterDividendsPerIssuerView',
      },
      {
        label: id,
      },
...
export default useRegisterDividendsPerIssuerView
