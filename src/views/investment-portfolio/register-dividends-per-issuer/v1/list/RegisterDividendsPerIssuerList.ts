...
const useRegisterDividendsPerIssuerList = () => {
  const { openMainLoader } = useMainLoader()
  const { defaultIconsLucide } = useUtils()
  const route = useRoute()

  const {
    register_dividends_per_issuer_list,
    register_dividends_per_issuer_pages,
  } = storeToRefs(useRegisterDividendsPerIssuerStore('v1'))
  const { _listAction } = useRegisterDividendsPerIssuerStore('v1')
  ...
  const headerProperties = {
    title: 'Registro de dividendos por emisor acciones',
    breadcrumbs: [
  ...
  const tableProperties = ref({
    ...
    columns: [
      ...
      },
      {
        ...
        align: 'left',
        label: 'Valor dividendo después de impuesto',
    ...
    pages: register_dividends_per_issuer_pages,
  })
  ...
  const loadData = async (filters: string = 'paginate=1') => {
    ...
    const hasResults = register_dividends_per_issuer_list.value.length > 0
  ...
  const handleOptions = (option: string, id: number) => {
    if (option === 'edit') handleGoTo('RegisterDividendsPerIssuerEdit', id)
    else if (option === 'view') handleGoTo('RegisterDividendsPerIssuerView', id)
  }
  ...
  watch(
    () => register_dividends_per_issuer_list.value,
    () => {
      tableProperties.value.rows = register_dividends_per_issuer_list.value
      tableProperties.value.pages = register_dividends_per_issuer_pages.value
    }
  )
...
export default useRegisterDividendsPerIssuerList
