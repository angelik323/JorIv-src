...
import {
  ...
  useLocalPurchaseExchangeTradedFundStore,
} from '@/stores'

const useETFLocalBuyCreate = () => {
  const { openMainLoader } = useMainLoader()
  const { defaultIconsLucide } = useUtils()

  const { _getResources, _resetKeys } = useResourceManagerStore('v1')
  const { _createAction } = useLocalPurchaseExchangeTradedFundStore('v1')
  ...
  const headerProperties = {
    title: "Crear compra ETF's moneda local",
    breadcrumbs: [
      {
        label: 'Inicio',
        route: 'HomeView',
  ...
  const handleSubmitForm = async () => {
    ...
    const payload: IEtfLocalBuyOperation = {
      ...
      value_unit_buy: compliance.unit_value,
...
export default useETFLocalBuyCreate
