...
const usePaymentInstructionsCreate = () => {
  // Composables
  ...
  const { _updatePaymentInstructions, _getPaymentInstructionsById } =
  ...
  // Configs
  const headerProps = {
    title: 'Crear instrucciones de pago',
    breadcrumbs: [
      {
        label: 'Inicio',
        route: 'HomeView',
      ...
      {
        label: 'Crear',
        route: 'PaymentInstructionsCreate',
      },
    ],
  }
  ...
  const makePayload = () => {
    ...
    return {
      ...
      currency_id: f?.currency_id ? String(f?.currency_id) : null,
  ...
  const handleCreate = async () => {
    if (!headerForm.value.payment_request_id) return

    const payload = makePayload() as IPaymentInstructionsPayload
    openMainLoader(true)
    if (headerForm.value.instruction_id) {
      if (
        await _updatePaymentInstructions(
          payload,
          headerForm.value.instruction_id
        )
      ) {
  ...
  watch(
    () => [headerForm.value.total_value, headerForm.value.iva_value],
    () => {
      const total = Number(headerForm.value.total_value ?? 0)
      const iva = Number(headerForm.value.iva_value ?? 0)

      if (instructionsForm.value) {
        instructionsForm.value.tax_discount = 0
        instructionsForm.value.base_value = total - iva
        instructionsForm.value.net_value = total - iva
      }
      foreignCurrencyForm.value.net_value = total - iva
    }
  )
  ...
  watch(
    ...
    async (val) => {
      ...
      foreignCurrencyForm.value = {
        ...
        final_value_foreign:
          instructionsForm.value?.final_value_foreign ?? null,
...
export default usePaymentInstructionsCreate
