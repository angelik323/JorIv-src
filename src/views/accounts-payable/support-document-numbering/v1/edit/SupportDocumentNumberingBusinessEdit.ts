...
export const useSupportDocumentNumberingBusinessEdit = () => {
  ...
  const headerProps = {
    title: 'Editar resolución por negocio',
    breadcrumbs: [
      ...
      {
        label: 'Editar',
        route: 'SupportDocumentNumberingBusinessEdit',
      },
      {
  ...
  const createPayload = (form: ISupportDocumentNumberingBusinessForm) => {
    return {
      business_trust_id: supportDocumentNumberingBusinessId,
      support_document_numbering_resolution_id:
        supportDocumentNumberingResolutionId,
      handles_issuer_data: form.handles_issuer_data,
      prefix: form.prefix,
      status_id: form.status_id,
    }
  }
  const handleSubmit = async () => {
    const isValid = await businessDataFormRef.value?.validateForm()
    if (!isValid) return
    if (!business_data_form.value) return
    openMainLoader(true)
    const payload = createPayload(business_data_form.value)
    if (await _updateSupportDocumentNumberingBusiness(payload)) {
...
export default useSupportDocumentNumberingBusinessEdit
