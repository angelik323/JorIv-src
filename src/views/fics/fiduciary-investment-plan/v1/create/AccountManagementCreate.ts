...
const useAccountManagementCreate = () => {
  ...
  const searchThirdParties = async () => {
    ...
    if (models.value.identification_number) {
      await _getResources(
        { third_party: ['third_parties'] },
        `sort=id&include=legalPerson,financialInfo,naturalPerson,estate,documentType,contacts,addresses,status&filter[is_customer]=true&fields[]=id,document,is_customer,third_party_category,document_type_id,status_id&filter[document]=${models.value.identification_number}`
      )
    }
