...
// Composables
import { useGoToUrl, useUtils, useMainLoader, useAlert } from '@/composables'
...
const useFiduciaryInvestmentPlanCreate = () => {
  ...
  const { _createFiduciaryInvestmentPlan, _clearData } =
  ...
  const makePayload = () => {
    return {
      ...
      holder_identification: {
        holder_id: data_form.value?.holder_identification.holder_id,
        fic_print_group_id:
