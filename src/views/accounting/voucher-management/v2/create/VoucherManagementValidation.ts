...
const useVoucherManagementValidation = () => {
  const { defaultIconsLucide } = useUtils()
  const { openMainLoader } = useMainLoader()
  const { goToURL } = useGoToUrl()

  const { headerPropsDefault } = storeToRefs(useVoucherManagementStore('v1'))
  const { _validateVoucher, _createValidation, _exportXlsxErrors } =
  ...
  const headerProperties = {
    ...
    breadcrumbs: [
      ...headerPropsDefault.value.breadcrumbs,
      {
        ...
        route: 'VoucherManagementValidation',
      },
    ],
  }
  ...
  const textForFieldProcessRealized = computed(() => {
    ...
    return isSuccessValidate.value ? 'Validación' : 'Validación con errores'
  })
  ...
  const onSubmit = async () => {
    ...
    const data_basic_data =
      voucherManagementFormRef.value?.getValues() as IVoucherManagementValidationForm
    ...
    if (isSuccessValidate.value) {
      ...
      const createValidationPayload = {
        ...data_basic_data,
        vouchers_ids: responseValidation.data[0] as number[],
      }
      if (await _createValidation(createValidationPayload)) {
  ...
  const downloadAction = async () => {
    ...
    await _exportXlsxErrors(
      toRaw(tableErrorsProps.value.rows),
      'Comprobantes_errados'
    )
    openMainLoader(false)
  }
...
export default useVoucherManagementValidation
