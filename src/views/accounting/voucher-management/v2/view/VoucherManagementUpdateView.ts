...
const useVoucherManagementUpdateView = () => {
  ...
  const updateId = Number(router.currentRoute.value.params.id)
  ...
  const headerProperties = {
    ...
    title: 'Ver actualización de comprobantes',
    breadcrumbs: [
      ...headerPropsDefault.value.breadcrumbs,
      {
        label: 'Ver actualización',
        route: 'VoucherManagementUpdateView',
      },
      {
        label: updateId.toString(),
      },
    ],
  }
  ...
  onMounted(async () => {
    ...
    const voucherWithErrors = await _getVoucherWithErrorsById(updateId)
...
export default useVoucherManagementUpdateView
