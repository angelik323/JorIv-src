...
import { IChartAccountFileItem } from '@/interfaces/customs'
...
const useChartAccountsImport = () => {
  ...
  const { total_records, url_file } = storeToRefs(useChartAccountsStore('v2'))
  ...
  const tableProps = ref({
    ...
    rows: [] as IChartAccountFileItem[],
  })

  const invalidImport = ref(false)
  const addedFiles = async (file: IUploadedFile[]) => {
    statusImport.value = false
    invalidImport.value = false
    ...
    const interval = setInterval(async () => {
      if (!statusImport.value) {
        ...
        if (state === 'Procesado') {
          ...
          tableProps.value.rows[0].total = Number(total_records.value || 0)
        }
        if (state === 'Errores') {
          clearInterval(interval)
