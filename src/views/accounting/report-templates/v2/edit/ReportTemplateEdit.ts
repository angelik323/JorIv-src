...
const useReportTemplateEdit = () => {
  ...
  const { _updatePartialReportTemplate, _getShowReportTemplate } =
    useReportTemplatesStore('v2')
  ...
  const setValueModel = (data: IReportTemplatePayload) => {
  ...
  const headerProperties = {
    title: 'Editar plantilla de informes contables',
    breadcrumbs: [
      ...headerPropsDefault.value.breadcrumbs,
      {
        label: 'Editar',
        route: 'ReportTemplateEdit',
      },
      {
  ...
  const onSubmit = async () => {
    ...
    if (!(await _updatePartialReportTemplate(idReference, payload))) {
  ...
  onMounted(async () => {
    ...
    await _getShowReportTemplate(idReference)
    openMainLoader(false)
  })
...
export default useReportTemplateEdit
